<x-app-layout>
    ...
    <div class="w-full min-w-0 max-w-6xl bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow mx-auto overflow-hidden">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center mb-6 min-w-0">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar produto</h1>
        ...
        <form action="{{ url('products/update') }}" method="POST" enctype="multipart/form-data" class="max-w-xl">
            @csrf

            <input type="hidden" name="id" value="{{ $product->id }}">

            <x-meu-input name="name" label="Nome:" type="text" value="{{ old('name', $product->name) }}" required />
            <x-meu-input name="description" label="Descrição:" as="textarea" rows="3" cols="40">{{ old('description', $product->description) }}</x-meu-input>
            <x-meu-input name="quantity" label="Quantidade:" type="number" value="{{ old('quantity', $product->quantity) }}" min="0" required />
            <x-meu-input name="price" label="Preço:" type="number" value="{{ old('price', $product->price) }}" step="0.01" min="0" required />

            <p class="block mb-1 text-gray-700 dark:text-gray-300">Imagem atual:</p>
            <img src="{{ asset('storage/' . $product->image) }}" alt="Imagem do produto" class="w-24 h-24 object-cover rounded mb-4 border border-gray-300 dark:border-gray-600">
            <label class="block mb-1 text-gray-700 dark:text-gray-300">Nova imagem (opcional):</label>
            <input name="image" type="file" accept="image/*" class="w-full p-2 mb-4 rounded border dark:bg-gray-700 dark:text-white" />
