<x-app-layout>
    ...
    <div class="w-full min-w-0 max-w-6xl bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow mx-auto overflow-hidden">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center mb-6 min-w-0">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar fornecedor</h1>
        ...
        <form action="{{ route('fornecedores.update') }}" method="POST" class="max-w-xl">
            @csrf
            <input type="hidden" name="id" value="{{ $supplier->id }}">

            <x-meu-input name="name" label="Nome:" type="text" value="{{ old('name', $supplier->name) }}" required />
            <x-meu-input name="email" label="E-mail:" type="email" value="{{ old('email', $supplier->email) }}" />
            <x-meu-input name="phone" label="Telefone:" type="text" value="{{ old('phone', $supplier->phone) }}" maxlength="32" />
