<x-app-layout>
    ...
    <div class="w-full min-w-0 max-w-6xl bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow mx-auto overflow-hidden">
        ...
        @endif
        @if (session('error'))
            <p class="mb-4 rounded-md bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 px-4 py-3 text-red-800 dark:text-red-200 text-sm">
                {{ session('error') }}
            </p>
        @endif
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center mb-6 min-w-0">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ url('/types/new') }}" class="shrink-0 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-center">Novo tipo</a>
                <a href="{{ url('/products') }}" class="shrink-0 bg-gray-200 dark:bg-gray-600 text-gray-900 dark:text-white px-4 py-2 rounded hover:bg-gray-300 dark:hover:bg-gray-500 text-center">Ver produtos</a>
            </div>
        </div>

        <div class="w-full min-w-0 overflow-x-auto">
            <table class="w-full min-w-[480px] table-auto border-collapse border border-gray-300 dark:border-gray-600">
                <thead>
                ...
                <tbody>
                    @forelse ($types as $type)
                        <tr class="border-b border-gray-300 dark:border-gray-600">
                            ...
                            <td class="px-4 py-2 text-gray-900 dark:text-white">{{ $type->products_count }}</td>
                            <td class="px-4 py-2">
                                <div class="flex flex-wrap justify-center gap-2">
                                    <a href="{{ url('/types/update', ['id' => $type->id]) }}" class="bg-gray-600 text-white px-3 py-1 rounded hover:bg-gray-700 text-sm">Editar</a>
                                    <a href="{{ url('/types/delete', ['id' => $type->id]) }}" class="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700 text-sm" onclick="return confirm('Excluir este tipo?');">Excluir</a>
                                </div>
                            </td>
                        </tr>
                    @empty
